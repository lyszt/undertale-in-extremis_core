...
\begin{resumo}
  ...
  compilado em um binário estático RISC-V e simulado no gem5. Do arquivo
  stats.txt foram extraídas a taxa de erro de predição, o IPC, o tempo de
  simulação e as instruções descartadas. O BiModeBP reduziu a taxa de erro de 11,27\%
  para 8,57\%, aumentou o IPC em 3,8\% e cortou em 38\% o trabalho
  especulativo descartado. Um segundo experimento mostra que essa vantagem
  depende de quanto a carga é previsível.
\end{resumo}
...
Seguindo o Tópico~2 da proposta, sobre branch prediction, o objetivo é
comparar dois preditores condicionais oferecidos pelo gem5
\cite{binkert2011gem5}, o
LocalBP, local e simples, e o BiModeBP, global e complexo. A carga usada é o
...
professor, no qual só o preditor condicional muda entre as rodadas, de
LocalBP para BiModeBP. O modelo de CPU em-ordem não rodou no ambiente usado,
então foi usado o modelo fora-de-ordem, também fornecido. Como só o preditor
muda, a comparação é justa. A seed do jogo vem do relógio, então as
contagens absolutas mudam um pouco a cada execução e a análise usa taxas,
que são estáveis.
...
\begin{table}[ht]
\centering
\caption{Métricas da carga mista, extraídas do stats.txt.}
\label{tab:metricas}
\begin{tabular}{lrr}
\toprule
Métrica & LocalBP & BiModeBP \\
\midrule
Desvios condicionais avaliados & 900.700 & 803.703 \\
Desvios condicionais incorretos & 101.531 & 68.838 \\
Taxa de erro de predição & 11,27\% & 8,57\% \\
IPC & 1,4603 & 1,5165 \\
Tempo simulado & 7,539\,ms & 7,181\,ms \\
Instruções efetivadas & 11.009.088 & 10.889.293 \\
Instruções descartadas & 102.245 & 62.953 \\
\bottomrule
\end{tabular}
\end{table}
...
Os resultados estão na Tabela~\ref{tab:metricas}. O BiModeBP errou 8,57\% dos
desvios condicionais contra 11,27\% do LocalBP, uma redução relativa de
24\%. Menos erros significam menos descartes, então o IPC subiu de 1,4603
para 1,5165, mais 3,8\%, e o tempo caiu 4,7\%. O efeito maior está nas
instruções descartadas, 62,9 mil no BiModeBP contra 102,2 mil no LocalBP,
38\% a menos.
...
com a taxa de erro subindo de 11,27\% para 12,59\% no LocalBP e de 8,57\%
para 10,07\% no BiModeBP, pois sortear a ação remove o viés dos desvios e um
desvio sem viés é imprevisível por construção. Além disso, a vantagem do
BiModeBP diminui de 24\% para 20\%. Quanto mais aleatória a carga, menor o
retorno do preditor complexo, cujo ganho vem da estrutura que a
aleatoriedade elimina.
...
O aumento de 3,8\% no IPC é modesto porque parte das decisões do jogo não
...
puramente aleatórias, os dois preditores pioram e a vantagem do BiModeBP cai
de 24\% para 20\%.
